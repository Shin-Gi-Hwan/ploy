...
Spring Boot + React + TypeScript 기반 풀스택 프로젝트
...
이를 통해 직접 URL 공유로 인한 파일 유출을 방지했습니다.
...
* MVP 단계에서 단순하고 빠른 구현 가능
* Stateless REST 구조와 잘 맞음
* React와 연동이 간단함
...
Spring Security 환경에서 단순 @CrossOrigin만으로는
...
관리자 프로젝트 목록 조회 시 N+1 문제를 방지하기 위해:
...
이 프로젝트를 통해
비즈니스 흐름을 이해하고, 이를 안정적인 시스템으로 설계하는 능력을 보여주고자 했습니다.
